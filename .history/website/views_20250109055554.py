...
def car_owners_offer(request):
    # Initialize context with relevant data, e.g., all car owners
    context = {
        'car_owners': CarOwner.objects.all(),  # Fetch all car owners from the database
    }

    # Render the car-owners-offer.html template with the context data
    return render(request, 'website/car-owners-offer.html', context)
